...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2 &msg)
{
    ...
    if ((lidar_type == AIRY || lidar_type == VELO || lidar_type == OUSTER || lidar_type == PANDAR || lidar_type == ROBOSENSE) && cut_frame)
    {
...
#pragma omp parallel for
        ...
        if (!imu_en && !data_accum_finished && data_accum_start)
        {
            ...
            if (data_accum_finished)
            {
                ...
                {
                    imu_buffer[i]->header.stamp = rclcpp::Time(rclcpp::Time(imu_buffer[i]->header.stamp).seconds() - time_lag_IMU_wtr_lidar);
                }
