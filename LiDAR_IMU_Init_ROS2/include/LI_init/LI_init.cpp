...
void LI_Init::downsample_interpolate_IMU(const double &move_start_time) {
    ...
    for (size_t i = mean_filt_size; i < IMU_state_group_ALL.size() - mean_filt_size; i++) {
        V3D acc_real = Zero3d;
...
void LI_Init::Butter_filt(const deque<CalibState> &signal_in, deque<CalibState> &signal_out) {
    ...
    for (size_t i = butter.Coeff_size; i < sig_extended.size() - butter.extend_num; i++) {
...
void LI_Init::solve_Rotation_only() {
    ...
    problem_rot.AddParameterBlock(R_LI_quat, 4);
    problem_rot.SetManifold(R_LI_quat, new ceres::QuaternionManifold());
...
void LI_Init::solve_Rot_bias_gyro(double &timediff_imu_wrt_lidar) {
    ...
    problem_ang_vel.AddParameterBlock(R_LI_quat, 4);
    problem_ang_vel.SetManifold(R_LI_quat, new ceres::QuaternionManifold());
...
void LI_Init::solve_trans_biasacc_grav() {
    ...
    problem_acc.AddParameterBlock(R_GL0_quat, 4);
    problem_acc.SetManifold(R_GL0_quat, new ceres::QuaternionManifold());
