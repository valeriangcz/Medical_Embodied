...
double readAsDouble(const uint8_t *data, uint8_t datatype)
{
  switch (datatype)
  {
...
{
  static const char *kCandidates[] = {"timestamp", "time", "t"};
...
double timeToMsScale(double t_min, double t_max)
{
  ...
  if (t_max > 1e11 && span > 1.0)
  {
    return 1e-3;
  }
  if (span <= 1.0 && t_max <= 100.0)
  {
    return 1000.0;
  }
  if (span <= 200.0 && t_max <= 1000.0)
  {
    return 1.0;
  }
  return 1000.0;
...
void Preprocess::rslidar_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  const auto *f_x = findField(*msg, "x");
  const auto *f_y = findField(*msg, "y");
  const auto *f_z = findField(*msg, "z");
  if (!f_x || !f_y || !f_z)
  {
    return;
  }

  const auto *f_intensity = findField(*msg, "intensity");
  if (!f_intensity)
  {
    f_intensity = findField(*msg, "reflectivity");
  }
  const auto *f_ring = findField(*msg, "ring");
  const auto *f_time = findTimeField(*msg);
  ...
  if (f_time)
  {
    for (size_t i = 0; i < total; ++i)
    {
      ...
      const double t = readAsDouble(p + f_time->offset, f_time->datatype);
      if (!std::isfinite(t))
      {
        continue;
      }
  ...
  given_offset_time = has_any_time;
  const double ms_scale = has_any_time ? timeToMsScale(t_min, t_max) : 1.0;

  pl_surf.reserve(total / point_filter_num + 1);

  for (size_t i = 0; i < total; ++i)
  {
    if (static_cast<int>(i % point_filter_num) != 0)
    {
      continue;
    }

    const uint8_t *p = msg->data.data() + i * msg->point_step;
    const float x = static_cast<float>(readAsDouble(p + f_x->offset, f_x->datatype));
    const float y = static_cast<float>(readAsDouble(p + f_y->offset, f_y->datatype));
    const float z = static_cast<float>(readAsDouble(p + f_z->offset, f_z->datatype));
    ...
    if (f_intensity)
    {
      added_pt.intensity = static_cast<float>(readAsDouble(p + f_intensity->offset, f_intensity->datatype));
      if (!std::isfinite(added_pt.intensity))
      {
        added_pt.intensity = 0.f;
      }
    }
    ...
    if (has_any_time)
    {
      ...
      added_pt.curvature = static_cast<float>((t - t_min) * ms_scale);
    }
    else
    {
      (void)f_ring;
